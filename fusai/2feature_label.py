...
# all构造特征
print("all")
error_union = errortmp.drop("error",axis=1).merge(error[["id","time","station","error"]].rename(columns={"id":"id1","time":"time1"}),how = 'left',on=["station"])
...
for i in [0,5,10,30,60,"all"]:
    print(i)
    error_union = errortmp.drop("error", axis=1).merge(errortmp[["id", "time", "station", "error"]].rename(columns={"id": "id1", "time": "time1"}), how='left',on=["station"])
    error_union = error_union.loc[error_union["id"] != error_union["id1"]]
    if i==0:
...
error.to_csv(out+"data_label0809.csv",index=False)