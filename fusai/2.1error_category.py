...
error["time_last120"] = error["time"].apply(lambda x: x - relativedelta(minutes=+120))
error["time_last240"] = error["time"].apply(lambda x: x - relativedelta(minutes=+240))
error["time_last480"] = error["time"].apply(lambda x: x - relativedelta(minutes=+480))
error["time_last960"] = error["time"].apply(lambda x: x - relativedelta(minutes=+960))
error["time_last1920"] = error["time"].apply(lambda x: x - relativedelta(minutes=+1920))
...
error["time_future120"] = error["time"].apply(lambda x: x + relativedelta(minutes=+120))
error["time_future240"] = error["time"].apply(lambda x: x + relativedelta(minutes=+240))
error["time_future480"] = error["time"].apply(lambda x: x + relativedelta(minutes=+480))
error["time_future960"] = error["time"].apply(lambda x: x + relativedelta(minutes=+960))
error["time_future1920"] = error["time"].apply(lambda x: x + relativedelta(minutes=+1920))
...
for i in [0,5,10,30,60,120,240,480,960,1920,"all"]:
    print(i)
    ...
    for j in train1.main_class.value_counts().index:
        error_tmp = error_union.loc[error_union.main_class1 == j]
        error_tmp = error_tmp.groupby(["id","station","is_train"], as_index=False).agg(agg)
...
#可以发现相同时间出现相同的故障概率很高
# future特征构造
for i in [5,10,30,60,120,240,480,960,1920,"all"]:
    print(i)
    error_union = error.drop("error", axis=1).merge(error[["id", "time", "station", "error",'main_class','next_class']].rename(columns={"id": "id1", "time": "time1",'next_class':'next_class1','main_class':'main_class1'}), how='left',on=["station"])
    error_union = error_union.loc[error_union["id"] != error_union["id1"]]
    if i == "all":
...
del error["time_last120"]
del error["time_last240"]
del error["time_last480"]
del error["time_last960"]
del error["time_last1920"]
del error["time_future5"]
del error["time_future10"]
del error["time_future30"]
del error["time_future60"]
del error["time_future120"]
del error["time_future240"]
del error["time_future480"]
del error["time_future960"]
del error["time_future1920"]
del error["hour"],error["day"],error["day_of_week"]


error.to_csv(out+'gupeng0819.csv',index = False)